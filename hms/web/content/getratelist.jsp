...
String where = "where currencyid = "+currency.getCurrencyid();
...
int count = RateManager.getInstance().countWhere(where);
int total_pages = 0;
...
RateBean[] rates = RateManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<rates.length;i++)
            {
            %>
            <row id='<%=(rates[i].getRateid())%>'>
                <cell><![CDATA[<%=sdt.format(new Date(rates[i].getRatedate().getTime()))%>]]></cell>
                <cell><![CDATA[<%=dcc.format(rates[i].getValue().doubleValue())%>]]></cell>
            </row>
            <%
            }
    %>
