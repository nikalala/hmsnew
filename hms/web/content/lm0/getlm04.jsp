...
int count = RoomManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", roomid "+sord;

RoomBean[] rooms = RoomManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<rooms.length;i++)
            ...
            if(rooms[i].getStatus().booleanValue())
                status = "აქტიური";
            String smoke = "არა";
            if(rooms[i].getSmoke().booleanValue())
                status = "კი";
            
            RoomtypeBean rb = RoomtypeManager.getInstance().loadByPrimaryKey(rooms[i].getRoomtypeid());
            BedtypeBean bb = BedtypeManager.getInstance().loadByPrimaryKey(rooms[i].getBedtypeid());
            
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'ROOM' and id = "+rooms[i].getRoomid()+" order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'ROOM' and id = "+rooms[i].getRoomid()+" order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            %>
            <row id='<%=rooms[i].getRoomid()%>'>
                <cell><![CDATA[<%=rooms[i].getNickname()%>]]></cell>
                <cell><![CDATA[<%=rooms[i].getName()%>]]></cell>
                
                <cell><![CDATA[<%=rb.getName()%>]]></cell>
                <cell><![CDATA[<%=bb.getName()%>]]></cell>
                
                <cell><![CDATA[<%=rooms[i].getOrd()%>]]></cell>
                
                <cell><![CDATA[<%=smoke%>]]></cell>
