...
int count = AmenityManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", amenityid "+sord;

AmenityBean[] amenities = AmenityManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<amenities.length;i++)
            ...
            if(amenities[i].getStatus().booleanValue())
                status = "აქტიური";
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'AMENITY' and id = "+amenities[i].getAmenityid()+" order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'AMENITY' and id = "+amenities[i].getAmenityid()+" order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            %>
            <row id='<%=amenities[i].getAmenityid()%>'>
                <cell><![CDATA[<%=amenities[i].getName()%>]]></cell>
                <cell><![CDATA[<%=amenitytypes[amenities[i].getType().intValue()]%>]]></cell>
                <cell><![CDATA[<%=amenities[i].getOrd()%>]]></cell>
