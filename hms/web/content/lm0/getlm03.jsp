...
int count = BedtypeManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", bedtypeid "+sord;

BedtypeBean[] bedtypes = BedtypeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<bedtypes.length;i++)
            {
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'BEDTYPE' and id = "+bedtypes[i].getBedtypeid()+" order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'BEDTYPE' and id = "+bedtypes[i].getBedtypeid()+" order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            %>
            <row id='<%=bedtypes[i].getBedtypeid()%>'>
                <cell><![CDATA[<%=bedtypes[i].getNickname()%>]]></cell>
                <cell><![CDATA[<%=bedtypes[i].getName()%>]]></cell>
