...
String order = "order by "+sidx+", roomtypeid "+sord;

RoomtypeBean[] roomtypes = RoomtypeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<roomtypes.length;i++)
            {
            String web = "არა";
            if(roomtypes[i].getWeb().booleanValue())
                web = "კი";
            
            String amenities = "";
            
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'ROOMTYPE' order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'ROOMTYPE' order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            %>
            <row id='<%=roomtypes[i].getRoomtypeid()%>'>
                <cell><![CDATA[<%=roomtypes[i].getNickname()%>]]></cell>
                <cell><![CDATA[<%=roomtypes[i].getName()%>]]></cell>
                <cell><![CDATA[<%=roomtypes[i].getBaseadalt()%>]]></cell>
                <cell><![CDATA[<%=roomtypes[i].getBasechild()%>]]></cell>
                <cell><![CDATA[<%=roomtypes[i].getMaxadalt()%>]]></cell>
                <cell><![CDATA[<%=roomtypes[i].getMaxchild()%>]]></cell>
                <cell><![CDATA[<%=web%>]]></cell>
                <cell><![CDATA[<%=roomtypes[i].getColor()%>]]></cell>
                <cell><![CDATA[<%=amenities%>]]></cell>
                <cell><![CDATA[<%=roomtypes[i].getOrd()%>]]></cell>
            </row>
            <%
            }
    %>
