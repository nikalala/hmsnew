...
<% ExtrachargeBean[] extrachargeBeans = ExtrachargeManager.getInstance().loadAll(); %>
...
<form name="filter-form" id="filter-form">
  <table id="grid-table" style="margin: 10px;">
    ...
    <tr>
      ...
      <td colspan="2">
        <select class="dropdown" name="extracharge" id="extracharge">
          <option value="">-აირჩიეთ-</option>
          <% for (int i = 0; i <= extrachargeBeans.length - 1; i++) { %>
          <option value="<%=extrachargeBeans[i].getExtrachargeid()%>"><%=extrachargeBeans[i].getName()%>
          </option>
          <% } %>
        </select>
      </td>
