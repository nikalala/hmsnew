...
<% TransportationmodeBean[] transportationmodeBeans = TransportationmodeManager.getInstance().loadAll(); %>
  ...
  <table id="grid-table" style="margin: 10px;">
    <tr>
      <td style="width: 150px;">
        <label>Pick/Drop Data From:</label>
      </td>
      <td>
    ...
    <tr>
      ...
      <td colspan="2">
        <select class="dropdown" name="transmode" id="transmode">
          <option value="">-აირჩიეთ-</option>
          <% for (int i = 0; i <= transportationmodeBeans.length - 1; i++) { %>
          <option value="<%=transportationmodeBeans[i].getTransportationmodeid()%>"><%=transportationmodeBeans[i].getName()%>
          </option>
          <% } %>
        </select>
      </td>
    ...
    <tr>
      ...
      <td colspan="2">
        <select class="dropdown" name="pickUpDropOffType" id="pickUpDropOffType">
          <option value="">-აირჩიეთ-</option>
          <% for (int i = 0; i <= pickUpDropOffType.length - 1; i++) { %>
          <option value="<%=i%>"><%=pickUpDropOffType[i]%>
          </option>
          <% } %>
        </select>
      </td>
