...
int count = SeasonManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", seasonid "+sord;

SeasonBean[] seasons = SeasonManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<seasons.length;i++)
            ...
            if(seasons[i].getStatus().booleanValue())
                status = "აქტიური";
            
            String startdate = sdt1.format(seasons[i].getStartdate());
            String enddate = sdt1.format(seasons[i].getEnddate());
            
            String fromdate = sdt.format(seasons[i].getFromdate());
            String todate = sdt.format(seasons[i].getTodate());
            
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'SEASON' and id = "+seasons[i].getSeasonid()+" order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'SEASON' and id = "+seasons[i].getSeasonid()+" order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            %>
            <row id='<%=seasons[i].getSeasonid()%>'>
                <cell><![CDATA[<%=seasons[i].getNickname()%>]]></cell>
                <cell><![CDATA[<%=seasons[i].getName()%>]]></cell>
                
                <cell><![CDATA[<%=startdate%>]]></cell>
                <cell><![CDATA[<%=enddate%>]]></cell>
                <cell><![CDATA[<%=fromdate%>]]></cell>
                <cell><![CDATA[<%=todate%>]]></cell>
