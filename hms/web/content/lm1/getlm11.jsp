...
int count = TarifftypeManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", tarifftypeid "+sord;

TarifftypeBean[] tarifftypes = TarifftypeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<tarifftypes.length;i++)
            ...
            if(tarifftypes[i].getStatus().booleanValue())
                status = "აქტიური";
            
            String rtypes = "";
            
            TariffroomtypeBean[] rtps = TariffroomtypeManager.getInstance().loadByTarifftypeid(tarifftypes[i].getTarifftypeid());
            for(int j=0;j<rtps.length;j++){
                if(j > 0)  rtypes += ", ";
                RoomtypeBean rtp = RoomtypeManager.getInstance().loadByPrimaryKey(rtps[j].getRoomtypeid());
                rtypes += rtp.getName();
            }
            
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'TARIFFTYPE' and id = "+tarifftypes[i].getTarifftypeid()+" order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'TARIFFTYPE' and id = "+tarifftypes[i].getTarifftypeid()+" order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            %>
            <row id='<%=tarifftypes[i].getTarifftypeid()%>'>
                <cell><![CDATA[<%=tarifftypes[i].getNickname()%>]]></cell>
                <cell><![CDATA[<%=tarifftypes[i].getName()%>]]></cell>
                
                <cell><![CDATA[<%=tarifftypes[i].getNumnights()%>]]></cell>
                <cell><![CDATA[<%=tarifftypes[i].getMaxadolt()%>]]></cell>
                <cell><![CDATA[<%=tarifftypes[i].getMinnight()%>]]></cell>
                
                <cell><![CDATA[<%=rtypes%>]]></cell>
                
                <cell><![CDATA[<%=tarifftypes[i].getOrd()%>]]></cell>
