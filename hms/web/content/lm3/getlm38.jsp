...
String where = "where templatecategoryid > 0 ";
...
int count = TemplatecategoryManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", templatecategoryid "+sord;

TemplatecategoryBean[] templatecategoryes = TemplatecategoryManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<templatecategoryes.length;i++)
            {

            %>
            <row id='<%=templatecategoryes[i].getTemplatecategoryid()%>'>
                <cell><![CDATA[<%=templatecategoryes[i].getName()%>]]></cell>
            </row>
            <%
            }
    %>
