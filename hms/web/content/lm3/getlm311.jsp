...
int count = ReservationtypeManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", reservationtypeid "+sord;

ReservationtypeBean[] reservationtypes = ReservationtypeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<reservationtypes.length;i++)
            ...
            if(reservationtypes[i].getApproved().booleanValue())
                status = "აქტიური";
            
            String clr = "<img src=\"images/blank.gif\" align=\"middle\" width=\"70\" height=\"20\" style=\"background-color: "+reservationtypes[i].getColor()+";\"/>";
            %>
            <row id='<%=reservationtypes[i].getReservationtypeid()%>'>
                <cell><![CDATA[<%=reservationtypes[i].getName()%>]]></cell>
                <cell><![CDATA[<%=reservationtypes[i].getColor()%>]]></cell>
                <cell><![CDATA[<%=clr%>]]></cell>
