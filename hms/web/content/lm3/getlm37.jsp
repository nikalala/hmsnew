...
String where = "where discountid > 0 ";
...
int count = DiscountManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", discountid "+sord;

DiscountBean[] discounts = DiscountManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<discounts.length;i++)
            {
            String extracharge = "არა";
            if(discounts[i].getExtracharge().booleanValue())
                extracharge = "კი";
            
            String opendiscount = "არა";
            if(discounts[i].getOpendiscount().booleanValue())
                opendiscount = "კი";
            
            String sharer = "არა";
            if(discounts[i].getSharer().booleanValue())
                sharer = "კი";
            %>
            <row id='<%=discounts[i].getDiscountid()%>'>
                <cell><![CDATA[<%=discounts[i].getName()%>]]></cell>
                <cell><![CDATA[<%=discounts[i].getNickname()%>]]></cell>
                <cell><![CDATA[<%=discounttype[discounts[i].getDiscounttype().intValue()]%>]]></cell>
                <cell><![CDATA[<%=opendiscount%>]]></cell>
                <cell><![CDATA[<%=dc.format(discounts[i].getAmount().doubleValue())%>]]></cell>
                <cell><![CDATA[<%=extracharge%>]]></cell>
                <cell><![CDATA[<%=sharer%>]]></cell>
            </row>
            <%
            }
    %>
