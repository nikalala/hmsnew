...
String where = "where preferencetypeid > 0 ";
...
int count = PreferencetypeManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", preferencetypeid "+sord;

PreferencetypeBean[] preferencetypes = PreferencetypeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<preferencetypes.length;i++)
            ...
            if(preferencetypes[i].getStatus().booleanValue())
                status = "აქტიური";
            %>
            <row id='<%=preferencetypes[i].getPreferencetypeid()%>'>
                <cell><![CDATA[<%=preferencetypes[i].getName()%>]]></cell>
