...
String where = "where marketcodeid > 0 ";
...
int count = MarketcodeManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", marketcodeid "+sord;

MarketcodeBean[] marketcodes = MarketcodeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<marketcodes.length;i++)
            ...
            if(marketcodes[i].getStatus().booleanValue())
                status = "აქტიური";
            %>
            <row id='<%=marketcodes[i].getMarketcodeid()%>'>
                <cell><![CDATA[<%=marketcodes[i].getName()%>]]></cell>
