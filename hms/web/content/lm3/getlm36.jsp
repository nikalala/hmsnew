...
int count = PayoutManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", payoutid "+sord;

PayoutBean[] payouts = PayoutManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<payouts.length;i++)
            ...
            if(payouts[i].getStatus().booleanValue())
                status = "აქტიური";
            %>
            <row id='<%=payouts[i].getPayoutid()%>'>
                <cell><![CDATA[<%=payouts[i].getName()%>]]></cell>
