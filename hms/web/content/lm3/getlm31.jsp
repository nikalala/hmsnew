...
int count = CurrencyManager.getInstance().countWhere(where);
int total_pages = 0;
...
CurrencyBean[] currs = CurrencyManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<currs.length;i++)
            {
            String basic = "არა";
            if(currs[i].getBasic().booleanValue())
                basic = "კი";
            %>
            <row id='<%=currs[i].getCurrencyid()%>'>
                <cell><![CDATA[<%=currs[i].getName()%>]]></cell>
                <cell><![CDATA[<%=currs[i].getCode()%>]]></cell>
                <cell><![CDATA[<%=basic%>]]></cell>
            </row>
            <%
            }
    %>
