...
String where = "where vipstatusid > 0 ";
...
int count = VipstatusManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", vipstatusid "+sord;

VipstatusBean[] vipstatuses = VipstatusManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<vipstatuses.length;i++)
            ...
            %>
            <row id='<%=vipstatuses[i].getVipstatusid()%>'>
                <cell><![CDATA[<%=vipstatuses[i].getName()%>]]></cell>
            </row>
            <%
            }
    %>
