...
String where = "where extrachargeid > 0 ";
...
int count = ExtrachargeManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", extrachargeid "+sord;

ExtrachargeBean[] charges = ExtrachargeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<charges.length;i++)
            ...
            String web = "";
            %>
            <row id='<%=charges[i].getExtrachargeid()%>'>
                <cell><![CDATA[<%=charges[i].getName()%>]]></cell>
                <cell><![CDATA[<%=charges[i].getNickname()%>]]></cell>
                <cell><![CDATA[<%=dc.format(charges[i].getRate().doubleValue())%>]]></cell>
                <cell><![CDATA[<%=fixed%>]]></cell>
                <cell><![CDATA[<%=web%>]]></cell>
                <cell><![CDATA[<%=charges[i].getOrd()%>]]></cell>
            </row>
            <%
            }
    %>
