...
String where = "where templatesid > 0 ";
...
int count = TemplatesManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", templatesid "+sord;

TemplatesBean[] templates = TemplatesManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<templates.length;i++)
            ...
            if(templates[i].getStatus().booleanValue())
                status = "აქტიური";
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'TEMPLATES' and id = "+templates[i].getTemplatesid()+" order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'TEMPLATES' and id = "+templates[i].getTemplatesid()+" order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            String cname = "";
            TemplatecategoryBean cat = TemplatecategoryManager.getInstance().loadByPrimaryKey(templates[i].getTemplatecategoryid());
            %>
            <row id='<%=templates[i].getTemplatesid()%>'>
                <cell><![CDATA[<%=templates[i].getName()%>]]></cell>
                <cell><![CDATA[<%=cat.getName()%>]]></cell>
                <cell><![CDATA[<%=createdby.getLname()%> <%=createdby.getFname()%>]]></cell>
                <cell><![CDATA[<%=chdate%>]]></cell>
                <cell><![CDATA[<%=templates[i].getNote()%>]]></cell>
