...
int count = ReasonManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", reasonid "+sord;

ReasonBean[] reasons = ReasonManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<reasons.length;i++)
            ...
            if(reasons[i].getStatus().booleanValue())
                status = "აქტიური";
            %>
            <row id='<%=reasons[i].getReasonid()%>'>
                <cell><![CDATA[<%=reasons[i].getName()%>]]></cell>
                <cell><![CDATA[<%=reasoncategory[reasons[i].getCategory().intValue()]%>]]></cell>
