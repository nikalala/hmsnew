...
int count = IdtypeManager.getInstance().countWhere(where);
int total_pages = 0;
...
String order = "order by "+sidx+", idtypeid "+sord;

IdtypeBean[] idtypes = IdtypeManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<idtypes.length;i++)
            ...
            if(idtypes[i].getStatus().booleanValue())
                status = "აქტიური";
            %>
            <row id='<%=idtypes[i].getIdtypeid()%>'>
                <cell><![CDATA[<%=idtypes[i].getName()%>]]></cell>
