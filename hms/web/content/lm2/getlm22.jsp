...
String order = "order by "+sidx+", unitsid "+sord;

UnitsBean[] units = UnitsManager.getInstance().loadByWhere(where+" "+order+" "+limit);
%>
<rows>
    <page><%=ipg%></page>
    ...
    <%
        for(int i=0;i<units.length;i++)
            ...
            if(units[i].getStatus().booleanValue())
                status = "აქტიური";
            
            ChangelogBean[] chlog = ChangelogManager.getInstance().loadByWhere("where name = 'UNITS' and id = "+units[i].getUnitsid()+" order by changedate asc limit 1");
            PersonellBean createdby = PersonellManager.getInstance().loadByPrimaryKey(chlog[0].getChangebyid());
            chlog = ChangelogManager.getInstance().loadByWhere("where name = 'UNITS' and id = "+units[i].getUnitsid()+" order by changedate desc limit 1");
            String chdate = dtlong.format(chlog[0].getChangedate().getTime());
            String clr = "<img src=\"images/blank.gif\" align=\"middle\" width=\"70\" height=\"20\" style=\"background-color: "+units[i].getColor()+";\"/>";
            %>
            <row id='<%=units[i].getUnitsid()%>'>
                <cell><![CDATA[<%=units[i].getName()%>]]></cell>
                <cell><![CDATA[<%=units[i].getColor()%>]]></cell>
                <cell><![CDATA[<%=clr%>]]></cell>
