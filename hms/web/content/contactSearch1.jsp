...
<script>
    function selGuest(id){
        $.post("content/srch_getcontragentinfo.jsp",{ id: id },function(data){
            $("#walkin_search_contactinfo").html(data);
        });
    }
    
    function selGuestIn(id){
        $.post("content/srch_getcontragentinfo_json.jsp",{ id: id },function(data){
            if(data.status == 0) BootstrapDialog.alert(data.error);
            else {
                $("#wlakin_other_companyid").val(id);
    ...
    function contact_search(){
        $.post(
        "content/ajax/getContactSearch1.jsp",
        { 
            cid : <%=cid%>,
